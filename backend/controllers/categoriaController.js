...
class CategoriaController {
    ...
    async atualizar(req, res) {
        const codigo = req.params.codigo;
        const _codigo = String((await categoriaModel.findOne({ 'codigo': codigo }))._id);
        await categoriaModel.findByIdAndUpdate(String(_codigo), req.body);
        res.status(200).send();
    }
    ...
    async excluir(req, res) {
        const codigo = req.params.codigo;
        const _codigo = String((await categoriaModel.findOne({ 'codigo': codigo }))._id);
        await categoriaModel.findByIdAndRemove(String(_codigo));
        res.status(200).send();
    }
}
